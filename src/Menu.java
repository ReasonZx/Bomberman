...
public class Menu extends BasicGameState{
	...
	public void update(GameContainer container, StateBasedGame arg1, int delta) throws SlickException {
		...
		if((posX > exit_x && posX < exit_x + exitGame.getWidth()) && (posY > exit_y && posY < exit_y + exitGame.getHeight())) {	// ver tamanhos certos dos bot�es
			if(container.getInput().isMousePressed(container.getInput().MOUSE_LEFT_BUTTON)) {
				System.exit(0);
			}
		}
		
		if((posX > guest_x && posX < guest_x + Guest.getWidth()) && (posY > guest_y && posY < guest_y + Guest.getHeight())) {	// ver tamanhos certos dos bot�es
			if(container.getInput().isMousePressed(container.getInput().MOUSE_LEFT_BUTTON)) {
				sbg.enterState(sbg.Get_Game_State());
			}
		}
		
		if((posX > signup_x && posX < signup_x + 280) && (posY > signup_y && posY < signup_y + 18)) {	// ver tamanhos certos dos bot�es
			highlighted = 1;
			if(container.getInput().isMousePressed(container.getInput().MOUSE_LEFT_BUTTON)) {
